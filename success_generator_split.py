...
def mix(hp, args, audio, num, s1_dvec, s1_target, s2, train):
    ...
    target_wav_path = formatter(dir_, "-target.wav", num)
    mixed_wav_path = formatter(dir_, "-mixed.wav", num)
    emb_wav_path = formatter(dir_, "-ref_emb.wav", num)
    sf.write(target_wav_path, w1, srate)
    sf.write(mixed_wav_path, mixed, srate)
    sf.write(emb_wav_path, d, srate)

    # save magnitude spectrograms
    target_mag, _ = audio.wav2spec(w1)
    mixed_mag, _ = audio.wav2spec(mixed)
    emb_mag, _ = audio.wav2spec(d)
    target_mag_path = formatter(dir_,"-target.pt", num)
    mixed_mag_path = formatter(dir_, "-mixed.pt", num)
    emb_mag_path = formatter(dir_, "-emb.pt", num)
    torch.save(torch.from_numpy(target_mag), target_mag_path)
    torch.save(torch.from_numpy(mixed_mag), mixed_mag_path)
    torch.save(torch.from_numpy(emb_mag), emb_mag_path)
