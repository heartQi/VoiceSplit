...
def train_wrapper(num):
    ...
    try:
        mix_wavfiles(output_dir_train, sample_rate, audio_len, ap, form, num, embedding_utterance_path,
                     interference_utterance_path, clean_utterance_path)
    except:
        print("Error, probably because one of these samples doesn't exist: ", clean_utterance_path,
              embedding_utterance_path, interference_utterance_path)


def test_wrapper(num):
    clean_utterance_path, embedding_utterance_path, interference_utterance_path = test_data[num]
    try:
        mix_wavfiles(output_dir_test, sample_rate, audio_len, ap, form, num, embedding_utterance_path,
                     interference_utterance_path, clean_utterance_path)
    except:
        print("Error, probably because one of these samples doesn't exist: ", clean_utterance_path,
              embedding_utterance_path, interference_utterance_path)
...
args.config = "/Users/mervin.qi/Desktop/PSE/Workspace/VoiceSplit/config.json"
args.dataset_root_dir = "/Users/mervin.qi/Desktop/PSE/Dataset/voicesplit_normalize"
args.train_data_csv = False
args.test_data_csv = "/Users/mervin.qi/Desktop/PSE/Workspace/VoiceSplit/datasets/LibriSpeech2/test_data_csv.csv"
args.out_dir = "/Users/mervin.qi/Desktop/PSE/Dataset/voicesplit_normalize/preprocess"
args.librispeech = False  # 或者 True，根据您的需要设置
...
if args.librispeech:
    ...
    if train_data_csv is not None:
        for c, e, i in train_data_csv:
            train_data.append([os.path.join(dataset_root_dir, c), os.path.join(dataset_root_dir, e),
                               os.path.join(dataset_root_dir, i)])
    if test_data_csv is not None:
        for c, e, i in test_data_csv:
            test_data.append([os.path.join(dataset_root_dir, c), os.path.join(dataset_root_dir, e),
                              os.path.join(dataset_root_dir, i)])
